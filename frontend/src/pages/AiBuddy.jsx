...
export default function AiBuddy() {
  ...
  return (
    <div className="flex flex-col mh-bg-primary" style={{ height: 'calc(100vh - 4rem)' }}>
      {/* Header */}
      <div className="px-6 py-4 border-b mh-border flex items-center gap-3 mh-bg-card flex-shrink-0">
        <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: 'var(--accent-gradient)' }}>
          <Bot size={20} className="text-white" />
        </div>
        <div>
          <h1 className="font-bold mh-text-primary">AI Buddy</h1>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: connected ? 'var(--success)' : 'var(--error)' }} />
            <span className="text-xs mh-text-tertiary">{connected ? 'Online' : 'Offline'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            {suggestions.map((s) => (
              <button key={s} onClick={() => setInput(s)}
                className="mh-card p-4 text-sm text-left hover:!border-[var(--accent)] cursor-pointer group">
                <Sparkles size={14} className="inline mr-1.5 mh-text-accent" />
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
            {msg.role === 'ai' && (
              <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'var(--accent-gradient)' }}>
            ...
            {msg.role === 'user' && (
              <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mh-bg-tertiary mh-text-secondary">
                <User size={16} />
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 animate-fade-in">
            ...
            <div className="mh-card !shadow-none px-4 py-3 rounded-2xl rounded-bl-sm">
              <div className="flex gap-1.5">
                <span className="w-2 h-2 rounded-full animate-bounce mh-text-tertiary" style={{ animationDelay: '0s', backgroundColor: 'var(--text-tertiary)' }} />
      ...
      {/* Input */}
      <div className="px-4 sm:px-6 py-4 border-t mh-border mh-bg-card flex-shrink-0">
        <form onSubmit={handleSend} className="flex items-center gap-2 p-2 rounded-2xl border mh-border mh-bg-input">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
            placeholder="Ask me anything..."
            className="flex-1 px-3 py-2.5 text-sm bg-transparent outline-none mh-text-primary" />
          <Button type="submit" size="sm" disabled={!input.trim()} className="!rounded-xl !px-4">
            <Send size={16} />
          </Button>
        </form>
      </div>
